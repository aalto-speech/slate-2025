...
#SBATCH --mem=32G
#SBATCH --time=1:30:00
#SBATCH --output=output-files/out-Baseline_CornLoss-HF_trainer-4.out
...
### Define Work Dir
working_directory="/m/triton/work/porwala1/slate_models/Baseline_CornLoss-TrainData"
preds_folder=$working_directory/preds

### create dir if doesn't exist.
mkdir -p $working_directory
mkdir -p $working_directory/figures
mkdir -p $preds_folder

### where the models are saved
model_dir="/m/triton/scratch/elec/t405-puhe/p/porwala1/slate_baseline_CornLoss-TrainData"
mkdir -p $model_dir
...
for part in 1 3 4 5; do
    ...
    python3 -u /m/triton/work/porwala1/slate_models/HF_baseline_CornLoss.py \
        ...
        --epochs ${epochs} \
        --part ${part} \
        --image_folder $working_directory/figures \
        --save_preds_folder "${preds_folder}" \
        --wandb_project_name "Baseline_CornLoss-TrainData-P${part}"
