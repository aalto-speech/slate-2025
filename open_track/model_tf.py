...
def sinusoidal_position_encoding(seq_len: int, d_model: int, device: torch.device):
    """
    Generate a [seq_len, d_model] matrix of positional encodings
    ...
    # Calculate the angles for each position and dimension
    angle_rates = 1 / (10000 ** (2 * (i // 2) / d_model))
    angles = pos * angle_rates  # [seq_len, d_model]

    # Apply sin to even indices, cos to odd indices
    # i.e. channel 2i gets sin, channel 2i+1 gets cos
...
class MultiAudioWhisperModel(nn.Module):
    def __init__(self, whisper_name="openai/whisper-small", chunk_duration=30,
                 nhead=4, num_layers=2, ff_multiplier=4):
        super().__init__()
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            activation='relu',
            dropout=0.05,
            batch_first=False  # (S, N, E) format
        )
        self.transformer_agg = nn.TransformerEncoder(
            encoder_layer,
    ...
    def forward(self, part1, part3, part4, part5, labels=None):
       ...
       # Loop over each sample in the batch
        for i in range(batch_size):
            ...
            for audio in audio_parts:
                ...
                for j in range(num_chunks):
                    ...
                    chunk_embs_all.append(chunk_embed)

            # Stack chunk embeddings for this part (not averaging) 
            if len(chunk_embs_all) == 0:
                # If for some reason the audio part is empty, use a zero vector
                combined_embed = torch.zeros((1, self.whisper.config.d_model), device=device)
            else:
                # Stack chunk embeddings => shape [seq_len, hidden_size]
                chunk_embs_tensor = torch.cat(chunk_embs_all, dim=0)  # cat along 0
                seq_len = chunk_embs_tensor.shape[0]

                # Add positional encoding
                pe = sinusoidal_position_encoding(seq_len, chunk_embs_tensor.shape[1], device)
                chunk_embs_tensor = chunk_embs_tensor + pe  # (seq_len, hidden_size)

                # Transformer expects shape (seq_len, 1, hidden_size) => unsqueeze dim=1 for batch=1
                chunk_embs_tensor = chunk_embs_tensor.unsqueeze(1)

                # Pass through aggregator => (seq_len, 1, hidden_size)
                transformer_out = self.transformer_agg(chunk_embs_tensor)

                # seq_len is the number of chunks, we can mean-pool over it
                # to get a single embedding for the entire audio part
                # Mean-pool => shape (1, hidden_size)
                combined_embed = transformer_out.mean(dim=0)
        ...
        preds = torch.cat(predictions, dim=0).view(batch_size) # shape: (batch_size,) as a 1D tensor of scores
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="whisper_finetune_output")
    parser.add_argument("--run_name", type=str, default="whisper_finetune")
